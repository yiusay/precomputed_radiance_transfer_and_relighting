...
bool isQuantization = false;

vector< vector<SparseMatrixEntry> >      TWt[6][3]; // for each cubemap's each color component
vector< vector<SparseMatrixEntry1Byte> > TWt_1byte[6][3]; // for each cubemap's each color component
...
void display()
{
        ...
        for(int j = 0; j < CUBEMAP_RESOLUTION; j++){
            for(int k = 0; k < CUBEMAP_RESOLUTION; k++){
                ...
                if(!isQuantization){
                    ...
                    weight += TWt_1byte[i][0][col].size() * fabs(wavelet_light[i][0][col]);
                    weight += TWt_1byte[i][1][col].size() * fabs(wavelet_light[i][1][col]); 
                    weight += TWt_1byte[i][2][col].size() * fabs(wavelet_light[i][2][col]);       
                }
    ...
    for(int i = 0; i < window_width*window_height*3; i++) buffer[i] = 0.0;
    ...
    for(unsigned int i = 0; i < n_used_light; i++){
        ...
        if(!isQuantization){
            ...
            for(int j = 0; j < 3; j++){
                for(unsigned int k = 0; k < TWt_1byte[light_pixel->cubemap_id][j][light_pixel->col].size(); k++){
                    char  val = TWt_1byte[light_pixel->cubemap_id][j][light_pixel->col][k].val;
                    int   row = TWt_1byte[light_pixel->cubemap_id][j][light_pixel->col][k].row;
    ...
    if(isQuantization){
        for(int i = 0; i < window_width*window_height*3; i++) buffer[i] *= 0.00787401575;  // 0.00787401575 = 1/127
    }
...
int main(int argc, char *argv[])
{
    ...
    if(argc == 2 && atoi(argv[1]) == 1) isQuantization = true;
    ...
    for(int i = 0; i < 6; i++){ // for each cubemap
        ...
        for(int j = 0; j < 3; j++){ // for each color component
            ...
            if(!isQuantization) TWt[i][j].resize(CUBEMAP_RESOLUTION*CUBEMAP_RESOLUTION);
            else                TWt_1byte[i][j].resize(CUBEMAP_RESOLUTION*CUBEMAP_RESOLUTION);
            ...
            while(true){
                vector<double> col_data(window_width*window_height, 0.0);
                ...
                for(int k = 0; k < n_rows_in_column; k++){
                    ...
                    if(!isQuantization) TWt[i][j][col].push_back(data);
                    else                col_data[data.row] = data.val;  
                }

                if(isQuantization){ // quantization and dithering

                    for (int m = 0; m < window_height; m++){
                        for (int n = 0; n < window_width; n++) {

                            if( fabs(col_data[m*window_width+n]) > 1.0e-6){

                                double quantized_value = (int)(col_data[m*window_width+n]*127.0); 

                                if(fabs(col_data[m*window_width+n]*127.0-quantized_value) > 0.5){
                                    if(col_data[m*window_width+n] > 0.0) quantized_value += 1.0;
                                    else                                 quantized_value -= 1.0;
                                }

                                static double scale = 1.0/127.0;
                                double quantization_error = (col_data[m*window_width+n]*127.0-quantized_value) * scale;

                                if(n+1 < window_width)                        col_data[m*window_width + n+1]     += quantization_error * 0.4375;
                                if(m+1 < window_height && n > 0)              col_data[(m+1)*window_width + n-1] += quantization_error * 0.1875;
                                if(m+1 < window_height)                       col_data[(m+1)*window_width + n]   += quantization_error * 0.3125;
                                if(m+1 < window_height && n+1 < window_width) col_data[(m+1)*window_width + n+1] += quantization_error * 0.0625;

                                col_data[m*window_width+n] = quantized_value; // 8-bit quantization
                            }
                    ...
                    for(int k = 0; k < window_width*window_height; k++){
                        if(fabs(col_data[k]) > 1.0e-6) TWt_1byte[i][j][col].push_back( SparseMatrixEntry1Byte(k, (char)col_data[k]) );
                    }
    ...
    n_used_light = CUBEMAP_RESOLUTION*CUBEMAP_RESOLUTION*6;
