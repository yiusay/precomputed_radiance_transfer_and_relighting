...
int FindNearestIntersection(double *org, double *dir, int current_object_index, double *hit_point)
{
    ...
    int    closest_f_id;
    double t_closest, u_closest, v_closest;

    
    if( kdtree.GetRayIntersectingTriangles(org, dir, closest_f_id, t_closest, u_closest, v_closest, mesh.vertices, mesh.faces) ){
        // ray hits model 
        if(t_closest > EPSILON && t_closest < nearest_t){
            nearest_t = t_closest;
            nearest_object_index = closest_f_id + 7;
        }
    }
...
void GetMaterialProperty(double *hit_point, int object_index, double *object_normal, double *object_diffuse)
{
    if(object_index == 0){
        ...
        object_normal[1] = 1.0;
        object_normal[2] = 0.0;
        object_diffuse[0] = 0.8; 
        object_diffuse[1] = 1.0;
        object_diffuse[2] = 0.8; 
    }else{
        for(int i = 0; i < 3; i++) object_normal[i] = mesh.faces[object_index-7].normal[i];
        object_diffuse[0] = object_diffuse[1] = object_diffuse[2] = 1.0; 
    }
...
void GetPixelColor(int cubemap_id, int *cubemap_pixel_coord, unsigned int *pixel_color, double view_theta, double view_phi)
{
        ...
        for(int i = 0; i < 6; i++){
            ...
            for(int j = 0; j < CUBEMAP_RESOLUTION; j++){
                for(int k = 0; k < CUBEMAP_RESOLUTION; k++){
                    ...
                    //if(i == 0 || i == 1){
                    //    retrieve_vector[0] = (cube_bbmax[0]-cube_bbmin[0]) * (j+0.5)*inv_cubemap_resolution - 256;
                    //    retrieve_vector[2] = (cube_bbmax[2]-cube_bbmin[2]) * (k+0.5)*inv_cubemap_resolution - 256;
                    //    if(i == 0) retrieve_vector[1] = cube_bbmax[1] - 256;
                    //    else       retrieve_vector[1] = cube_bbmin[1] - 256; 
                    //}else if(i == 2 || i == 4){
                    //    retrieve_vector[0] = (cube_bbmax[0]-cube_bbmin[0]) * (j+0.5)*inv_cubemap_resolution - 256;
                    //    retrieve_vector[1] = (cube_bbmax[1]-cube_bbmin[1]) * (k+0.5)*inv_cubemap_resolution - 256;
                    //    if(i == 2) retrieve_vector[2] = cube_bbmin[2] - 256;
                    //    else       retrieve_vector[2] = cube_bbmax[2] - 256;
                    //}else{ // i == 3 || i == 5
                    //    retrieve_vector[2] = (cube_bbmax[2]-cube_bbmin[2]) * (j+0.5)*inv_cubemap_resolution - 256;
                    //    retrieve_vector[1] = (cube_bbmax[1]-cube_bbmin[1]) * (k+0.5)*inv_cubemap_resolution - 256;
                    //    if(i == 3) retrieve_vector[0] = cube_bbmax[0] - 256;
                    //    else       retrieve_vector[0] = cube_bbmin[0] - 256;
                    //}
                    if(i == 0 || i == 1){
    ...
    if(isNewLighting){    
        ...
        for(int i = 0; i < 6; i++){
            ...
            unsigned int raw_pixel_data[CUBEMAP_RESOLUTION+4][CUBEMAP_RESOLUTION+4][3];
            ...
            for(int j = 0; j < CUBEMAP_RESOLUTION; j++){
                for(int k = 0; k < CUBEMAP_RESOLUTION; k++){
                 ...
                 //   cerr << "theta bef " << theta/PI*180.0 << "  ";
                    ...
                    else if(pixel_coord_y >= png_height) pixel_coord_y = png_height-1;

              //      cerr << "theta aft " << theta/PI*180.0 << endl;

                    int pixel_index = (pixel_coord_y*png_width + pixel_coord_x) * 3;

             //       for(int m = 0; m < 3; m++) raw_pixel_data[j+2][k+2][m] = png_data[pixel_index+m];
            ...
            /*
            // fill pixel with no data with black (+-pixels for margin)
            for(int j = 0; j < 2; j++){
                for(int k = 0; k < CUBEMAP_RESOLUTION+4; k++){
                    for(int m = 0; m < 3; m++){
                        raw_pixel_data[j][k][m] = raw_pixel_data[CUBEMAP_RESOLUTION+2+j][k][m] = 0;
                        raw_pixel_data[k][j][m] = raw_pixel_data[k][CUBEMAP_RESOLUTION+2+j][m] = 0;
                    }
                }
            }
            ...
            int gaussian_filter[5][5] = { {1,  4,  6,  4, 1 }, 
                                          ...
                                          {4, 16, 24, 16, 4 }, 
                                          {1,  4,  6,  4, 1 }, };
            double weight = 0.00390625;

            for(int j = 0; j < CUBEMAP_RESOLUTION; j++){
                for(int k = 0; k < CUBEMAP_RESOLUTION; k++){

                    for(int m = 0; m < 3; m++){              
                        filtered_pixel_data[i][j][k][m] = 0;

                        for(int x = 0; x < 5; x++){
                            for(int y = 0; y < 5; y++){
                                filtered_pixel_data[i][j][k][m] += raw_pixel_data[j+x][k+y][m] * gaussian_filter[x][y];
                            }
                        }

                        filtered_pixel_data[i][j][k][m] *= weight;
                    } // for(int m = 0; m < 3; m++){  
...
void TracePathWithImportanceSampling(double *eye, double *ray, int current_object_index, double *color, int &cubemap_id, int *cubemap_pixel_coord, double weight_of_path, int depth)
{
    ...
    double hit_point[3], object_normal[3], object_diffuse[3];
    bool   isLight = false;

 
     if( (nearest_object_index = FindNearestIntersection(eye, ray, current_object_index, hit_point)) != -1 ){

        GetMaterialProperty(hit_point, nearest_object_index, object_normal, object_diffuse);
        ...
        if(nearest_object_index < 6){
            ...
            else           light_intensity_scale = 6.0;
            ...
            double weight = inv_probability_to_kill;

            TracePathWithImportanceSampling(hit_point, incoming_direction, nearest_object_index, incoming_color, cubemap_id, cubemap_pixel_coord, weight_of_path*weight, depth+1);
...
void OutputWeightIntoFile(GLfloat *image, ofstream **path_tracing_data)
{
    ...
    for(unsigned int i = 0; i < mesh.vertices.size(); i++){
        ...
        GetDirectionUsingCosWeightedLambertian(mesh.vertices[i].point, mesh.vertices[i].normal, ray);
        Normalize(ray);
        ...
        TracePathWithImportanceSampling(mesh.vertices[i].point, ray, -1, color, cubemap_id, cubemap_pixel_coord, 1.0, 0);
        ...
        if(cubemap_id != -1){ // ray hits one of the cubemaps
            ...
            image[i*3]    += (GLfloat)(color[0]*pixel_color[0]*scale);
            image[i*3 +1] += (GLfloat)(color[1]*pixel_color[1]*scale);
            image[i*3 +2] += (GLfloat)(color[2]*pixel_color[2]*scale);
        ...
        static ofstream fout("temp");
        //fout << (int)cubemap_id << " " << row << " " << col << " " << weight[0] << " " << weight[1] << " " << weight[2] << endl;
    }
    ...
    for(unsigned int i = 0; i < table_vertices.size(); i++){
        ...
        GetDirectionUsingCosWeightedLambertian(table_vertices[i].point, table_vertices[i].normal, ray);
        Normalize(ray);
        ...
        TracePathWithImportanceSampling(table_vertices[i].point, ray, 6, color, cubemap_id, cubemap_pixel_coord, 1.0, 0);
        ...
        if(cubemap_id != -1){ // ray hits one of the cubemaps
            ...
            static float scale = 1.0/255.0;
            image[(mesh.vertices.size()+i)*3]    += (GLfloat)(color[0]*pixel_color[0]*scale);
            image[(mesh.vertices.size()+i)*3 +1] += (GLfloat)(color[1]*pixel_color[1]*scale);
            image[(mesh.vertices.size()+i)*3 +2] += (GLfloat)(color[2]*pixel_color[2]*scale);


            unsigned int   row = mesh.vertices.size()+i;
            unsigned short col = cubemap_pixel_coord[1]*CUBEMAP_RESOLUTION+cubemap_pixel_coord[0];
            float          weight[3];
    ...
    glVertex3d( cube_bbmin[0], cube_bbmin[1], cube_bbmin[2] );
    glVertex3d( cube_bbmax[0], cube_bbmin[1], cube_bbmin[2] );
    glVertex3d( cube_bbmax[0], cube_bbmax[1], cube_bbmin[2] );
    glVertex3d( cube_bbmin[0], cube_bbmax[1], cube_bbmin[2] );
    glEnd();

    glBegin( GL_LINE_LOOP );
    glVertex3d( cube_bbmin[0], cube_bbmin[1], cube_bbmax[2] );
    glVertex3d( cube_bbmax[0], cube_bbmin[1], cube_bbmax[2] );
    glVertex3d( cube_bbmax[0], cube_bbmax[1], cube_bbmax[2] );
    glVertex3d( cube_bbmin[0], cube_bbmax[1], cube_bbmax[2] );
    glEnd();
